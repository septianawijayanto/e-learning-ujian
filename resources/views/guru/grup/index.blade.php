    ...
    <table class="table align-items-center table-flush mytable">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{$dt->status}}</td>
                <td>
                    <a href="{{url('guru/grup/'.$dt->id.'/edit')}}" class="btn btn-warning btn-sm btn-flat"><i class="fas fa-pencil-alt"></i> Edit </a>
                    <a href="{{url('guru/grup/'.$dt->id.'/delete')}}" class="btn btn-danger btn-sm btn-flat" onclick="return confirm('Apakah Akan Anda Hapus?')"><i class="far fa-times-circle"></i> Hapus </a>
                    <a href="{{url('guru/grup/'.$dt->id.'/detail')}}" class="btn btn-success btn-sm btn-flat"><i class="far fa-eye"></i> Show </a>
                </td>
            </tr>
            @endforeach
        </tbody>
            ...
            <div class="modal-body">
                <form action="{{ route('grup.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group {{$errors->has('kode') ? 'has-error' :''}}">
                        ...
                        <input name="kode" type="text" class="form-control" id="exampleInputkode1" readonly placeholder="Input kode" value="{{$kode}}">
                        @if($errors->has('kode'))
